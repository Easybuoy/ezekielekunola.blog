---
title: Deploying Vue App from Github to Netlify
author: Ezekiel Ekunola
date: 2020-02-28
hero: ./images/preview.png
excerpt: Learn about deploying vue app to netlify.
---

I will be doing a walk-through process of deploying a Vue application from Github to Netlify.

If you have a project structure where the vue app is **NOT** nested in a sub-directory like in the example shown below

![](./images/project-structure.png)

You can use the following steps to deploy:

- Push your code to Github
- Connect your Github account to your Netlify account
- Select the project you want to deploy
- Deploy project

See example in the image below

![](./images/vue-deploy.gif)

In a case where your vue application is available in a nested directory like the example shown below

![](https://thepracticaldev.s3.amazonaws.com/i/9th5maami9dct1ttqhw7.gif)

Add a **`netlify.toml`** file to the root of your project, and add the configuration below to the file.

    [build]
      command = "npm run build"
      publish="path-to-vue-app/dist"
      base = "path-to-vue-app"

## Routing

If you've made use of routing in your project, on redirecting and reloading to a new route, you may run into issue where Netlify would throw an error ("page not found") as shown in the example beloww

![](./images/vue-broken-url.gif)

You can add the following code to your `netlify.toml` file.

    [[redirects]]
      from = "/*"
      to = "/index.html"
      status = 200

Once added, push the changes to Github. Netlify should automatically deploy the changes, if not, deploy the changes manually. Once deployed, your application should not throw errors, as shown below.

![](./images/vue-router-fixed.gif)

**Note:** _`netlify.toml` is a configuration file where you can specify how Netlify should build/run your application. A lot can be accomplished with the `netlify.toml` file. Find out more about the `netlify.toml` file [here.](https://www.netlify.com/docs/netlify-toml-reference/)_

## Environment Variables

You can decide to add the environment variable at the point of deploying? or at any other point in time

If you've created the vue application using vue-cli, you need to add the `VUE_APP` prefix to all your environment variables.

![](./images/vue-deploy-environment-variable.gif)

If you've made use of environment variables in your project, you can also add them to Netlify by following the steps shown in the image below

![](./images/vue-environment-variables.gif)

## Conclusion

From the foregoing, we have seen and been able to demonstrate that hosting a vue application through Github on Netlify is pretty simple and seamless, hence can be done in little or no time.

If you have any questions or feedback about this article, feel free to reach out.

Thanks for reading.
