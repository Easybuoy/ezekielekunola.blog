...
A package.json file is a JSON file that exists at the root of a project and holds metadata relevant to the project. It is mostly used for managing the project's dependencies and scripts.

## Why is the package.json file needed
The package.json file is needed and used by NPM or Yarn to store the names and versions of packages installed.
...
There are still so much more properties that can be specified in the package.json file, if you want to go in-depth, check [here](https://docs.npmjs.com/files/package.json).
